...
def cobre_test_data(n_subs=10):
    ...
    timeseries = [np.load(os.path.join(ts_path, p))
                  for p in sorted(os.listdir(ts_path))[:n_subs]]
    sub_ids = [int(p.split('_')[1])
               for p in sorted(os.listdir(ts_path))[:n_subs]]
    connectomes = [np.load(os.path.join(conn_path, p))
                   for p in sorted(os.listdir(conn_path))[:n_subs]]
...
def fake_data(n_roi=15, n_timepoints=150, n_subs=10, seed=111):
    ...
    sub_ids = np.random.choice(
        list(range(n_subs*2)), size=n_subs, replace=False).tolist()
...
class TestFeatures:
    ...
    def test_knn_graph_valid(self):
        A = np.array([[1.6, 0.15, -0.1, -0.7], [0.15, -2.3, 0.75, -0.6],
                     [-0.1, 0.75, 1.5, -0.5], [-0.7, -0.6, -0.5, -1.1]])
        B = np.array([[0., 0.075, 0., -0.7], [0.075, 0., 0.75, -0.6],
                     [0, 0.75, 0., -0.25], [-0.7, -0.6, -0.25, 0.]])
...
class TestModels:
    def test_gcn(self):
        ...
        val_loader = DataLoader(
            data, batch_size=batch_size, sampler=val_sampler)

        # Create model
        gcn = GCN(data.graph.edge_index, data.graph.weight,
                  n_timepoints=n_timepoints)

        # Train and evaluate model
        loss_fn = nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(
            gcn.parameters(), lr=0.001, weight_decay=0.0005)
