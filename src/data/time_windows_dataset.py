"""
str(__file__) \
+ "\ngcn_package v{} - class {}".format(src.__version__, self.__class__.__name__) \
...
class TimeWindowsDataset(torch.utils.data.Dataset):
  def __init__(
    ...
    ----------
      data_dir: `string`
        Input data directory from where to read intermediate data (already splitted timeseries and associated labels).
      partition: `string`
        Which partition to select, from "train", "valid" or "test" (default: \"train\").
      val_ratio: `float`
      ...
      autoencoder: `bool`
        Make the generated target same as generated data, for auto-encoding (default: False).
    """
    ...
    if ((self.test_ratio + self.val_ratio) >= 1.0):
      raise ValueError("Test and validation ratio are greater than one: {:.2f} > 1.0 !".format(self.test_ratio + self.val_ratio))
    valid_partition_names = ["train", "valid", "test"]
    if self.partition not in valid_partition_names:
      raise ValueError("Invalid partition name '{}', available partition names are {}.".format(self.partition, valid_partition_names))
  ...
  def __repr__(self):
    return "{}*({}, {})".format(self.__len__(), self.__getitem__(0)[0].shape, self.__getitem__(0)[1].shape)
